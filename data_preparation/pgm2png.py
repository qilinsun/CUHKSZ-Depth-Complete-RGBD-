...
def convert_pgm_P5(in_path, out_path):
    ...
    with open(in_path, 'rb') as f:
        # 读取两个字节 - 幻数，并解码成字符串
        magic_number = f.readline().strip().decode('utf-8').split(' ')
        if not operator.eq(magic_number[0], "P5"):
            raise Exception("该图像有误")
        # 读取高和宽
        width, height = magic_number[1], magic_number[2]
        width = int(width)
        height = int(height)
        # 读取最大值
        maxval = magic_number[3]
        # 每次读取灰度值的字节数
        if int(maxval) < 256:
            one_reading = 1
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_dir', default='data_1219/lib/origin_raw_dep_pgm', type=str, help='Dir to the pgm files')
    parser.add_argument('--output_dir', default='data_1219/lib/origin_raw_dep', type=str, help='Dir to the result files')
