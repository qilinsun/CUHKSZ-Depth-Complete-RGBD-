...
def depth_rgb_align(depth_path, align_depth_path, rgb_path, aligned_depth_rgb_path):
    ...
    R = camera_info.rotation_matrix
    T = camera_info.translation_matrix
    # print(np.max(depth))
    ...
    for i in range(depth.shape[0]):
        for j in range(depth.shape[1]):
            ...
            if (dep != 0):
                ...
                P_rgb_3d = np.dot(R.T, P_dep_3d) - np.dot(R.T, T) # Transform d3 point from depth camera space to rgb cam space.
                p_rgb_2d = np.dot(camera_info.rgb_intrinsic, P_rgb_3d) # Porject the 3d point into rgb cameara plane
                dep_rgb = P_rgb_3d[2]
                # print(dep_rgb)
                # input()
...
if __name__ == '__main__':
    ...
    aligned_depth_rgb_folder = "align_depth_rgb"
